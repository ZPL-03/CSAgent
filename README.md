...
运行时数据写入 `data/runtime/`，该目录属于本地运行产物，不进入 Git。PyQt6 主界面已经包含“智能体流程”页，可读取运行时事件库并展示节点状态、人工确认点、LLM 后端配置状态、LLM 调用轨迹、有限元作业队列和工具调用审计。LLM 调用轨迹只记录后端名称、模型名称、调用状态、耗时和错误摘要，不记录 URL、密钥、系统提示词或用户提示词。重大阶段仍以源码、配置、测试和文档入库。
